  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] p-6 flex justify-between items-center pointer-events-none">
      <div className="flex items-center gap-2 pointer-events-auto bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
        <div
          className={`w-8 h-8 ${isAdventureMode ? 'bg-orange-600' : 'bg-blue-600'} rounded-full flex items-center justify-center transition-colors duration-500`}
        >
          <span className="text-white font-bold">G</span>
        </div>
        <span className={`font-bold ${isAdventureMode ? 'text-white' : 'text-slate-900'}`}>
          Gopalakrishnan C
        </span>
      </div>
      ...
      <div className="pointer-events-auto flex items-center gap-4">
        {!isAdventureMode ? (
          <a
            ...
            className="bg-white/50 backdrop-blur-md p-3 rounded-full hover:bg-blue-500 hover:text-white transition-all text-slate-600 border border-slate-200"
          >
            <Linkedin size={18} />
          </a>
        ) : (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="bg-white/10 backdrop-blur-md p-3 rounded-full hover:bg-pink-500 hover:text-white transition-all text-white/70 border border-white/10"
          >
            <Instagram size={18} />
          </a>
        )}
        ...
        >
          <div
            className={`absolute inset-1 w-24 rounded-full transition-all duration-500 flex items-center justify-center gap-2 ${
              isAdventureMode
                ? 'translate-x-[88px] bg-orange-600 text-white'
